...
控えめ・意味のある動きのみ (CSS 主体、一部 Motion、`prefers-reduced-motion` 対応必須)。3D/パララックス、および背景全面を覆うグラデーションは封印。ヒーローの fluid orb (輪郭を持つ 1 個の形) のみ例外とする (改訂 2026-09-08)。
...
| 項目 | 当初 | 改訂後 | 理由 |
|------|------|--------|------|
| Hero | 製品デモ (額装した Explorer スクショ) を Hero に置く | **大タイポのみ**。スクショは直後の Preview セクションへ移す | tagline を `Launcher × Explorer` に確定したことで、Hero は 1 行の宣言に集中させた方が強い。正直主義は Preview セクションで維持する |
| モーション | グラデ/3D/パララックスを封印 | 3D/パララックスは封印のまま。**輪郭を持つ 1 個の形 (ヒーローの fluid orb) のみ例外** | 全面グラデーションは一度実装して差し戻した (白地では滲みが汚れに見えた)。一方、円形に閉じた orb は「意図して置かれた物体」として読めて成立する。**境界は面か形かであって、シェーダーを使うか否かではない** |
...
| スクロール | (規定なし) | **スクロールスナップと `html { scroll-behavior: smooth }` を使わない** | セクション単位の吸着 → 上下 2 点だけ → 全廃と 2 段階で差し戻した。`proximity` でも吸着点の近傍でホイールが引っ張られ、root の `smooth` はキーボード / スクロールバー操作まで再タイミングする。**ホイールの感触は自作しない**。アンカー遷移のイージングだけをクリックハンドラで与える |
