...
控えめ・意味のある動きのみ (CSS 主体、一部 Motion、`prefers-reduced-motion` 対応必須)。3D/パララックス、および背景全面を覆うグラデーションは封印。ヒーローの fluid orb (輪郭を持つ 1 個の形) のみ例外とする (改訂 2026-09-08)。**その fluid orb は Hero の作り直しで撤去し、例外はヒーローのパネルリング (ホバーでのせり出し / 開くときの回転) に移った** (改訂 2026-09-09)。動かしてよいのは状態を伝える場合に限る、という原則は変わらない。
...
| モーション | グラデ/3D/パララックスを封印 | 3D/パララックスは封印のまま。例外は **ヒーローのパネルリング** (改訂 2026-09-09。それまでは fluid orb) | 全面グラデーションは一度実装して差し戻した (白地では滲みが汚れに見えた)。一方、円形に閉じた orb は「意図して置かれた物体」として読めて成立した — **境界は面か形かであって、シェーダーを使うか否かではない**。Hero を作り直して orb の居場所 (右の余白) が無くなったため撤去し、代わりにリングの動きを例外とした。こちらは装飾ではなく、どのパネルを指しているか・開いたものがどこから来たかという**状態**を伝えている |
