...
控えめ・意味のある動きのみ (CSS 主体、一部 Motion、`prefers-reduced-motion` 対応必須)。グラデ/3D/パララックスは封印 (一度緩和を試みて差し戻した。改訂 2026-09-08)。
...
| モーション | グラデ/3D/パララックスを封印 | **封印のまま (変更なし)** | ヒーロー背景に極薄シェーダーを一度実装したが、白地の上では滲みが汚れに見え、ページの他セクション (罫線と余白だけで組む) と語彙が食い違った。当初の封印が正しかったため差し戻した |
