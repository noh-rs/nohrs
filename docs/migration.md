...
| **ピンを外す順序を固定する** | 500MB の枠は**既存のピンと取り込んだピンで共有**します (別枠にすると、import のたびに枠が増えて上限が意味を失います)。枠を超えたら**取り込んだ側から、`copied_at` が古い順に**ピンを外します。既存のピンには触りません — ユーザーが自分で付けたものを、移行が黙って外す方が驚きが大きいからです。同着は**アーカイブ内の並び順**で決めます — 配列の後ろにあるものから外します。`copied_at` は秒精度の移行元だと簡単に同着するので、順序が一意に決まる基準が別に要ります。**同じ初期状態と同じアーカイブなら、何度やっても同じピン状態になる**ことが要件です |
...
| **`copied_at` は必須** | 無い / 数値でない / 負の値のクリップは**適用前に拒否**し、どのレコードかを報告します。並び替えにも保持の判定にも使う値なので、欠けたまま既定値を当てると**取り込むたびに違う結果**になります。`docs/mif.schema.json` (§3.1 の最後) でも `required` にします |
| **クリップに移行元由来の `id` を持たせる** | 差分レポートは「どれを外したか」を示す必要があり、取り込み先の SQLite の `id` は**書いた後にしか存在しません**。MIF のクリップ記録に `id` を**必須**で持たせ、レポートはこれで指します。
  **移行元に安定な id があるならそれを使います** (再エクスポートしても同じ値になるもの)。無い移行元では importer が
  `source.app` + アーカイブ内の位置から作りますが、これは**そのアーカイブの中でだけ意味を持つ値**です —
  移行元が並びを変えれば次のエクスポートでは違う値になります。**アーカイブ横断の同一性には使えない**ことを
  `source` に明記し (`"clip_ids": "stable" | "archive-scoped"`)、`archive-scoped` のアーカイブ同士を
  突き合わせて「同じクリップ」と判定することはしません。
  どちらの場合も **`id` の重複は適用前に拒否**します (重複があるとレポートも対応表も曖昧になるため)。
  取り込み先の `id` との対応表は適用結果に含め、`noh migrate undo` がそれを使います |
