...
| **ピンを外す順序を固定する** | 500MB の枠は**既存のピンと取り込んだピンで共有**します (別枠にすると、import のたびに枠が増えて上限が意味を失います)。枠を超えたら**取り込んだ側から、`copied_at` が古い順に**ピンを外します。既存のピンには触りません — ユーザーが自分で付けたものを、移行が黙って外す方が驚きが大きいからです。同着は `id` の昇順で決めます。**同じ初期状態と同じアーカイブなら、何度やっても同じピン状態になる**ことが要件で、外した項目は `id` と理由を差分レポートに出します |
