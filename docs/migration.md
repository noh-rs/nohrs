...
```jsonc
{
  ...
  "clips":      [ { "id": "...", "kind": "text", "preview": "...", "body_ref": "blobs/0001.txt", "source_app": "com.apple.Safari", "copied_at": 0, "pinned": false } ],
...
| **`copied_at` は必須で、単位まで固定する** | 値は **Unix エポックからのミリ秒、UTC、整数**です。無い / 整数でない / 負 / 範囲外のクリップは**適用前に拒否**し、どのレコードかを報告します。「非負の数」とだけ決めて単位を書かないと、秒で出す移行元とミリ秒で出す移行元が混ざったときに**古い順が逆転**し、外れるピンが変わります。範囲は **2000-01-01 以降、取り込み時刻 +24 時間以内**です — 秒のつもりで入った値は 1970 年代に落ちるので、単位の取り違えはここで弾けます (未来側の 24 時間は時計ずれの余裕)。移行元が秒精度なら exporter が 1000 倍して書きます。`docs/mif.schema.json` (§3.1 の最後) にも `required` と型・範囲を書きます |
