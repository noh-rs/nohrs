...
| **クリップに移行元由来の `id` を持たせる** | 差分レポートは「どれを外したか」を示す必要があり、取り込み先の SQLite の `id` は**書いた後にしか存在しません**。MIF のクリップ記録に `id` (移行元で安定なもの。無ければ importer が `source` + アーカイブ内の位置から決定的に作る) を必須で持たせ、レポートはこれで指します。取り込み先の `id` との対応表は適用結果に含め、`noh migrate undo` がそれを使います |
