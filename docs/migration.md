...
```jsonc
{
  ...
  "stores": ["snippets", "quicklinks", "hotkeys", "aliases", "favorites", "clips", "notes", "unsupported"],
  "snippets":   [ { "id": "...", "name": "Email", "keyword": ";em", "body": "...", "placeholders": ["clipboard"] } ],
  "quicklinks": [ { "bound": { "command": "web.open", "args": {}, "title": "GitHub Search" },
                    "target": "https://github.com/search?q={argument}", "app": null } ],
  ...
  "clips":      [ { "id": "...", "kind": "text", "preview": "...", "body_ref": "blobs/0001.txt",
                    "source_app": "com.apple.Safari", "copied_at": 1789479920000, "pinned": false } ],
...
- **`stores` は「このアーカイブが持っていると主張するもの」の一覧**です。
  ...
  **「元から無かった」と「入っているはずが欠けている」が区別できません**。
  ...
  - `stores` に挙がっていて、**キーが有り値が配列**なら有効です。**空配列 `"clips": []` は正当**で、
    「このアーカイブはクリップを扱ったが 1 件も無かった」を意味します — それを言い分けるための
    `stores` なので、ここでエラーにしては本末転倒です。
  - `stores` に挙がっているのに**キーが無い / 配列でない** → **エラーで止めます** (壊れたアーカイブ)。
...
- **時刻の書き方が 2 つあるのは意図的です。** `source.exported_at` は人が読むためのメタデータなので
  **RFC3339 (オフセット付き)**、クリップの `copied_at` は並べ替えに使うので
  **Unix エポックからのミリ秒の整数**です (上の例の `1789479920000` は `2026-09-15T13:45:20Z`)。
  どちらも必須で、検証の規則は [§3.2](#32-適用-apply) の表にあります。
...
- **展開を始める前に、アーカイブ全体の大きさを見ます。** 1 アイテム 50MB / 総量 2GB は展開後の blob に
  ...
  次の 4 つを先に見て、どれか 1 つでも超えたら**1 バイトも書かずに**拒否します。
  ストリームしながら数え、途中で超えたらそこで止めます。

  | 上限 | 既定 | 無いとどうなるか |
  |------|-----:|------------------|
  | メンバ数 | 10 万 | 小さいメンバを大量に並べられる |
  | 圧縮後の総バイト数 | 500MB | 入力そのものが無制限になる |
  | 展開後の総バイト数 / 圧縮後の総バイト数 | 100 倍 | 高圧縮で膨らませられる |
  | **展開後の総バイト数 (絶対値)** | **4GB** | **上の 3 つを全部満たしても 500MB × 100 = 50GB が通ります**。比だけでは止まりません |

  拒否・失敗のどちらで終わってもステージングは消します。
- **メタデータの中身にも上限が要ります。** 上の 4 つはアーカイブのメンバに対するもので、
  **1 つの JSON メンバの中に何件入っているか**は縛れません。`clips` に 1000 万件並べた
  1 つのファイルは、メンバ数 1・圧縮後も小さい、で全部の網を抜けます。パースの前に:
  - **ストアごとの件数上限** (既定 10 万件)。超えたらそのストアを `unsupported` に落として続けます
    (アーカイブ全体を拒否はしません — 他のストアは移せるので)。
  - **デコード後の JSON 全体のバイト数** (既定 256MB) と、**1 フィールドの長さ** (既定 1MB)。
    どちらもストリーミングパーサで数えながら当て、超えた時点で止めます。
- **上に挙げた 7 種類は移行元から来るものだけ**です。`noh export --all` (G5) はこれに加えて
  ...
  **含まないもの**は再生成可能なキャッシュ (検索インデックス / サムネイル) だけで、それは export に入れません。
  ...
  取りに行く先は**アーカイブに書かれた URL** — 他人が書いた文字列なので、
  [`launcher-requirements.md`](./launcher-requirements.md) §5.9 の OGP 取得と**同じ規則をそのまま適用**します。
  スキームとプライベート IP の判定だけでは足りません — **リダイレクトのたびに解決し直し、検証し直し、
  検証した IP に接続を固定する**ところまでが 1 セットです。そこを抜くと、リダイレクト 1 回か
  DNS の張り替えで社内ホストに行けます。userinfo 付き URL は拒否、`Authorization` は送りません。
  **接続先 IP を固定できない HTTP クライアントしか使えないなら、この取得機能自体を作りません** (R24)。
  取得した実体は記録された sha256 と照合し、合わなければ捨てます。
...
- **このアーカイブは平文です。** クリップボードの中身・設定・履歴・シェルフ・plugin の KV が
  ...
  - **一時ファイルに書き、`fsync` してから rename し、rename のあと親ディレクトリも `fsync`** します。
    途中の中身が他から読める時間を作らず、ここで電源が落ちても「途中まで書けた zip」や
    「名前だけあって中身が無いファイル」が残りません。適用前スナップショットは**この直後に移行が走る**ので、
    ここが中途半端だと undo の戻り先が壊れます。
...
| **ピンを外す順序を固定する** | 500MB の枠は**既存のピンと取り込んだピンで共有**します (別枠にすると、import のたびに枠が増えて上限が意味を失います)。枠を超えたら**取り込んだ側から、`copied_at` が古い順に**ピンを外します。既存のピンには触りません — ユーザーが自分で付けたものを、移行が黙って外す方が驚きが大きいからです。同着は**アーカイブ内の並び順**で決めます — 配列の後ろにあるものから外します。`copied_at` は秒精度の移行元だと (ミリ秒に直しても) 簡単に同着するので、順序が一意に決まる基準が別に要ります。**同じ初期状態と同じアーカイブなら、何度やっても同じピン状態になる**ことが要件です |
| **既存のピンだけで枠を超えている場合** | 「枠は共有」と「既存のピンには触らない」は、**既存だけで 500MB を超えていると両立しません**。そのときは**取り込んだピンを全部外して**取り込み (アイテムは残る)、**既存の超過はそのまま**にします。移行が勝手に直すべきものではないからです。差分レポートに「既存のピンが枠を超えているため、取り込んだ N 件はピンなしで入れた」と 1 行出し、整理は通常のクリップボード UI に任せます |
| **`copied_at` は必須で、単位まで固定する** | 値は **Unix エポックからのミリ秒、UTC、整数**です。無い / 整数でない / 負 / 範囲外のクリップは**適用前に拒否**し、どのレコードかを報告します。「非負の数」とだけ決めて単位を書かないと、秒で出す移行元とミリ秒で出す移行元が混ざったときに**古い順が逆転**し、外れるピンが変わります。下限は **2000-01-01** — 秒のつもりで入った値は 1970 年代に落ちるので、単位の取り違えはここで弾けます。上限は **`source.exported_at` + 24 時間**で、**取り込み時刻は見ません**。「今から 24 時間以内」にすると、同じアーカイブが今日は通って明日は落ちる、dry-run と apply で判定が変わる、ということが起き、すぐ上の「何度やっても同じピン状態になる」と正面から衝突します。**判定に使う値はすべてアーカイブの中**に置きます (24 時間は、エクスポート直前のコピーを移行元が切り上げたときの余裕)。そのため `source.exported_at` も**必須**で、RFC3339 として解釈できなければ止めます — 上限の基準そのものが無くなるからです。`exported_at` が取り込み時刻より未来でも**拒否はしません**。時計のずれで取り込む中身が変わってはいけないので、差分レポートに 1 行出すだけで、結果は変えません。移行元が秒精度なら exporter が 1000 倍して書きます。`docs/mif.schema.json` (§3.1 の最後) にも `required` と型・範囲を書きます |
| **クリップに移行元由来の `id` を持たせる** | 差分レポートは「どれを外したか」を示す必要があり、取り込み先の SQLite の `id` は**書いた後にしか存在しません**。MIF のクリップ記録に `id` を**必須**で持たせ、レポートはこれで指します。**移行元に安定な id があるならそれを使います** (再エクスポートしても同じ値になるもの)。無い移行元では importer が `source.app` + アーカイブ内の位置から作りますが、これは**そのアーカイブの中でだけ意味を持つ値**です — 移行元が並びを変えれば次のエクスポートでは違う値になります。**アーカイブ横断の同一性には使えない**ことを `source` に明記し (`"clip_ids": "stable" \| "archive-scoped"`)、`archive-scoped` のアーカイブ同士を突き合わせて「同じクリップ」と判定することはしません。どちらの場合も **`id` の重複は適用前に拒否**します (重複があるとレポートも対応表も曖昧になるため)。取り込み先の `id` との対応表は適用結果に含め、`noh migrate undo` がそれを使います |
