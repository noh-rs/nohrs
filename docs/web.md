...
1. **Hero**: 5 枚の画面をリング状に置き、その中心に tagline (大タイポ) + サブコピー + 主 CTA を置く (**改訂 2026-09-09**。それまでは「製品スクショは Hero に置かない」= 大タイポのみだった)。実装は `components/HeroOrbit.tsx` + `.orbit*` (app.css)
   ...
   - **縦は 1 つの単位 (ステージ高) から引く**。パネルとタイポの大きさもこれに従う — 幅だけから引くとノート PC の縦で破綻する。一方 **横半径 (`--rx`) はウィンドウの半幅に追従させる**。これは意図で、どの幅でも左右のパネルが縁で切れるようにするため。狭い幅と縦長のウィンドウでは幾何そのものを差し替える (下記)
     ...
     - パネルはカードの中で拡大して**細部を見せる**。900px のウィンドウを 400px に縮めると白い矩形にしかならない。開くと同じ点を軸にズームが戻り、ウィンドウ全体が現れる (640px 未満では「全体」ではなく同じ点を中心とした縦長の切り抜き。下記)
     - 切り抜き枠は**画像の上端より上にはみ出してよい** (`SPILL`)。パネルの上端は必ず切られていて画面に出ないためで、はみ出た空白は見えない。アプリの中身はウィンドウ上部に集まっているので、この余裕がないと「下半分の空白」しか見せられなくなる
   - **開いたパネルは最終サイズで組み、カードの上に縮小して置く** (改訂 2026-09-09)。逆 (カードのサイズから拡大) にすると、読む側が見る状態の罫線と角丸が倍率ぶん太る。最初の 1 フレームだけは transition を切る — でないと「カードの位置へ移動するアニメーション」が先に走り、開くアニメーションはその途中から始まって何も動かない
     - 縮小率は**縦横のうち小さい方**から取る。枠がスクショと同じ形なら 2 つは同じ値になるが、枠を立てた画面 (下記) では横で合わせると最終サイズの 7 割から始まってしまい、「大きくなる」が見えない
   - ホバー / フォーカスでパネルが中心方向へ 56px せり出す。`prefers-reduced-motion` では全部即時
     - **せり出しの条件は「離れられるポインタ」と「キーボードが動かしたフォーカス」に限る** (改訂 2026-09-09)。`:hover` はタッチだとタップした要素に残り、`:focus-within` は閉じたときに元のカードへフォーカスを戻す実装と噛み合う。両方を無条件に効かせていたため、**スマホでは 1 タップごとにパネル 1 枚がコピーの上に 56px 出たまま残り**、別の場所をタップするまで戻らなかった。`@media (hover: hover) and (pointer: fine)` と `:has(.orbit-card:focus-visible)` で分ける。Esc ボタンで閉じた PC でも同じ症状が出ていた
     - この副作用は**閉じるアニメーションの着地もずらす**。閉じる直前に測ったカードの位置へパネルは戻るが、着地後にフォーカスが戻ってカードが 56px 動くため、パネルとカードが別の場所に並ぶ
   - **狭い幅でもリングは組む** (改訂 2026-09-09。当初は 1024px 未満を横スクロールの帯にしていた)。ただし**縦長のウィンドウ (`max-aspect-ratio: 5/4`) と 640px 未満ではリングを立てる**: ステージを縦に伸ばし、パネルは上下の縁から入り、左右はコピーの脇を細く通る。横長の値をそのまま使うとリングがウィンドウより広くなり、左右のパネルは角しか画面に残らない (768×1024 で実測 1,500px²。他が 20,000px² 前後)。逆に幅から引くとリングがコピーを囲い込み、コピーの置き場所が無くなる (中央に置けるのは 200×214px 程度、という計算になる)
     ...
     - **640px 未満では開いたパネルの枠も立てる** (改訂 2026-09-09)。900px のウィンドウを 370px の枠に丸ごと収めると、上の「白い矩形にしかならない」がそのまま起きる (アプリ内の文字が 3.3px)。しかも横長の枠は縦長の画面の 3 割しか覆わないので、**タップして開く意味が無くなっていた**。枠の高さを「キャプションと閉じるボタンの残り」まで伸ばし (`100svh - 220px`)、`object-fit: cover` で **リングと同じ `centre` を中心に横を切る**。画面の 83% を覆い、文字は 8px になる
       - 高さの床は**スクショ自身の形** (`--panel-w * 549 / 900`)。横長の窓 (横向きのスマホ) では切らずに全体を出す。天井は**スクショ自身の高さ (549px)** で、これは切り幅が狭くなりすぎるのも同時に止める。この 2 つで画像 1px ≒ CSS 1px に落ち、それが読める条件
       - 縦長の枠は横だけを切る (`cover` は 1 軸)。中身が上部に寄ったスクショでは下に空白が残るが、それはアプリの実際の空リストであり、両軸を切るには拡大が必要で、拡大すると窓の 2 割しか出せなくなる。**根本的な解は 900px より広い録画**
       - 切り抜き位置は枠の形から計算するので、**開いている間の枠のリサイズを `ResizeObserver` で見る**。閉じる飛行は `dismiss` が測り直すが、切り抜きは開いている状態そのものを決めるため測り直す口が無かった。横向きで開いて縦に回すと枠だけが立ち、切り抜きは窓の中央のまま残る (実測: `preview` は `centre` 0.75 に対し 0.295〜0.705 を表示 = 見せたい点が枠の外)
       - 縦長のウィンドウ (`max-aspect-ratio: 5/4`) では**枠は立てない**。タブレット幅ならウィンドウ全体が入り、切ると右のプレビュー枠が落ちる (768×1024 で実測)
