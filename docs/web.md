...
1. **Hero**: 5 枚の画面をリング状に置き、その中心に tagline (大タイポ) + サブコピー + 主 CTA を置く (**改訂 2026-09-09**。それまでは「製品スクショは Hero に置かない」= 大タイポのみだった)。実装は `components/HeroOrbit.tsx` + `.orbit*` (app.css)
   ...
   - **リングに置くのは実在の画面だけ** (改訂 2026-09-09)。素材は `assets/doc/demo.gif` から抜いたフレーム (`public/shots/*.png`、900×549、英語ロケール) で、Explorer / Preview / Search / Matches / Source の 5 状態。**Launcher・Plugin のパネルは作らない** — §6.1 の「実在する画面しか出さない」は Hero でも変わらないため
     - 理由は 2 枚で別 (改訂 2026-09-10)。**Plugin** はホストが未実装 (`crates/nohrs-pages/src/extensions.rs` が placeholder) なので、撮る画面がそもそも無い。**Launcher** は #256 で実装されたので撮れるが、`public/shots/` にまだ無い。**実物のフレームが入った時点で 6 枚目として足してよい** — その場合 `angleOf(index, 6)` が 60° 刻みになり、`--a` の逆回転を掛けている `.orbit-open` はそのまま追随する
   ...
   - **正直主義の対象は「載せる証拠」であって「文章の語り口」ではない** (改訂 2026-09-10)。リングに出すのは実在する画面のみ、Launcher/Plugins のパネルは**偽装せず mock も作らない** — ここは変えない。一方で**文章は完成した製品として書く**: サイトは「ある程度完成してから世に出す」前提で公開するので、pre-alpha の自己申告 (「まだ pre-alpha」「まだユーザーがいません」「スクショが無いので載せていません」) は置かず、製品として何であるかを書く
     ...
     - **ただし「今できること」を名乗る文は別** (改訂 2026-09-10)。製品が何であるかを述べる文 (tagline・`hero.sub`・`why` の 4 本柱・`note`) は完成形で書いてよいが、読者が**手元で今できることとして読む**文 — docs の `getting-started`、`plugins` の `installHint` — は**ツリーの実物に従わせる**。判定基準は「読んだ人がこれから 5 分でそれを試そうとするか」
       - **未実装として書くのはプラグインホスト (P4) だけ** (改訂 2026-09-10)。ランチャーは #256 で入った (`crates/nohrs-launcher/`、`Cmd+Shift+Space`)。プラグインホストは `crates/nohrs-pages/src/extensions.rs` が今も placeholder
       - **この判定はロードマップではなくツリーで行う**。一度ロードマップを根拠に「ランチャーは P3 で入る」と書き、その 11 分前にランチャーがマージされていたことがある。`ROADMAP.md` のフェーズ状態は手で更新されるデータで、実装より遅れる。`crates/` を見ること
