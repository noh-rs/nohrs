...
- ただし **輪郭を持つ 1 個の形は可** (改訂 2026-09-08)。全面の滲みは「汚れ」に見えるが、円に閉じた orb は「意図して置かれた物体」として読める。境界は面か形かであって、シェーダーを使うか否かではない
  - **fluid orb は撤去した** (改訂 2026-09-09)。Hero がリング構成になり「右側の余白」が無くなったため、中心に置くと文字の背後の滲みになった。上の原則 (面ではなく形) はそのまま有効で、対象が無くなっただけ
...
│   ├── components/            # Header / Footer / Section / Phases / HeroOrbit / DocsSearch …
...
1. **Hero**: 5 枚の画面をリング状に置き、その中心に tagline (大タイポ) + サブコピー + 主 CTA を置く (**改訂 2026-09-09**。それまでは「製品スクショは Hero に置かない」= 大タイポのみだった)。実装は `components/HeroOrbit.tsx` + `.orbit*` (app.css)
   ...
   - 幾何はすべて 1 つの単位 (ステージ高) から引く。**ウィンドウ幅から引くとノートPCの縦で破綻する**
     - パネルは楕円上、名前は同じ楕円をパネル半枚 + 余白ぶん内側に入れた位置。**名前に独立したリングを与えない** — 縦の短いウィンドウでは CTA との間に帯が残らず、ボタンに重なる
     - パネルも名前も同じ角度で回す。開くときはその角度が 0 に戻る回転として見える。角度は (-180, 180] に畳む (288° は逆回りで 72°)
     - パネルはカードの中で拡大して**細部を見せる**。900px のウィンドウを 300px に縮めると白い矩形にしかならない。開くと同じ点を軸にズームが戻り、ウィンドウ全体が現れる
     - パネルは必ず外側の縁で切られる = 見えているのは常に下半分。フレーミングの縦位置が揃っているのはそのため
   ...
   - 1024px 未満ではリングを組まず、CTA の下に横スクロールの帯として並べる。開く動きは同じ
