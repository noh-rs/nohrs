...
1. **Hero**: 5 枚の画面をリング状に置き、その中心に tagline (大タイポ) + サブコピー + 主 CTA を置く (**改訂 2026-09-09**。それまでは「製品スクショは Hero に置かない」= 大タイポのみだった)。実装は `components/HeroOrbit.tsx` + `.orbit*` (app.css)
   ...
   - **狭い幅でもリングは組む** (改訂 2026-09-09。当初は 1024px 未満を横スクロールの帯にしていた)。640px 未満だけ幾何を差し替え、**ステージを縦に伸ばす**: パネルは上下の縁から入り、左右はコピーの脇を細く通る。幅から引くとリングがコピーを囲い込み、コピーの置き場所が無くなる (中央に置けるのは 200×214px 程度まで、という計算になる)
     - **この幅では名前を出さない**。中央のコピーの角とパネルの内側の縁の間に、名前が乗る帯が残らない。パネル自体がボタンであり、名前は開いたときのキャプションに出る
