...
- **どちらも `nohrs-indexd` に依頼します。** 居なければ起動し、最後のクライアントが去って猶予時間 (既定 90 秒)
  ...
  ([ADR 0010](./adr/0010-indexd-owns-the-index-writer.md))。
- `daemon` 行が索引の**鮮度**を示します。`watching` なら誰かがファイルシステムの変更を追っている状態で、
  そうでなければ「このコマンドを打った時点の内容」でしかありません。件数からは分からない違いです。
  デーモンが居ても watcher の設置に失敗していることはあり (その場合もデーモンは終了しません)、
  そのときは `running but not watching` と出ます。「居ない」とは別の状態なので、別の言い方をします。
- `stop` は索引を消しません。更新を止めるだけで、検索はそのまま動きます (読み手は索引を直接読むので、
  デーモンの有無に関係なく答えられます)。
- 存在するが空のインデックスは、無いのと同じ扱いで報告します。どちらも検索を 0 件にするからです。
- Windows は unix socket が前提のこの経路を使えないため、その場で索引を更新します。デーモンに辿り着けない
  場合 (起動できなかったときも同様) も同じです。このとき tantivy の writer を取りに行くのは**その場で索引を
  更新するプロセス自身**であり、writer を他の誰かが握っていれば `build` は失敗します。握っている相手は
  開いているアプリとは限りません。起動には成功したが応答を待てなかったデーモンは、こちらから辿り着けないまま
  writer を持っています。いずれの場合も、その旨をそのまま表示します。
