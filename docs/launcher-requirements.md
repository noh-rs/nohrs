...
| **notch のメディア / HUD / ウィジェット (コアとして)** | [`notch.md`](./notch.md) §3.0 の T4。Control Center が同じものを 20px 隣で出しており、狭い席を取る価値がない。加えて Now Playing は非公開 API 依存。**plugin としては載せられる** (同 §3.9) ので、「作らない」のはコア実装だけ |
