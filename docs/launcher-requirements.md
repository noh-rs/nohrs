...
| # | 原則 | 具体的に意味すること |
|---|------|--------------------|
| **1** | **ローカルファースト / 無音** | ネットワークに出る機能は**すべてオプトイン**。既定でテレメトリなし・外部送信なし、AI も既定オフ (Tinycast の "Nothing turns on until you do" と同じ立場)。**アカウントは存在しますが、無くても全機能がローカルで動きます** — 「アカウント不要」は概念が無いという意味ではなく、**無くても使える**という意味です ([`device-sharing.md`](./device-sharing.md) §9) |
...
| 1.15 | リーダーキー (二段ショートカット) | — | — | ◎ | P3.5 | コマンドへの割り当てをいくつ増やしてもグローバル登録は 3 つのまま。3 つ自体が Wayland portal の上限に収まるかは R2 ([§5.10](#510-ショートカット)) |
...
| 5.12 | 設定の同期 (クラウド) | ✔ (Pro) | — | ✕ | — | [§10](#10-non-goals)。代わりに「同期可能なファイル配置」を保証。5.15 とは別物で、**設定は端末をまたぎません** — OS が違えば押せる組み合わせも違い、揃えると片方が壊れます |
| 5.13 | バックアップ / 復元 | ✔ | ✔ | ◎ | P3.5 | 平文 JSON。`noh export` / `noh import` |
| 5.14 | チーム共有 / SSO | ✔ | — | ✕ | — | [§10](#10-non-goals)。5.15 は**自分の端末どうし**で、他人と共有する機能ではありません |
| 5.15 | **端末間共有 (シェルフ / クリップボード)** | ✔ (Cloud Sync, Pro) | — | ○ | P5 | [`device-sharing.md`](./device-sharing.md)。**同じ LAN の自分の端末どうし**。同期先はサーバではなく自分のもう一台で、クラウドに置く 5.12 とは別です。アカウント無しでも手動ペアリングで動きます |
...
端末をまたぐ共有は [`device-sharing.md`](./device-sharing.md) §7 に分けてあります。
既定は**最新 1 件だけ**が相手の「いまのクリップボード」に届き、履歴そのものの共有はオプトインです。
ここに書く除外ルールと保持の上限は、**共有をオンにしても効きます** (同 §7.3 / §7.4)。
...
- `List` → `Split` の遷移中、検索バーの位置が **1px も動かない** — **1 枚のウィンドウで実装できた場合**。
  ウィンドウ 2 枚に倒す場合 (上記のとおり R1 / notch.md の R9 次第) はこの基準を満たせないので、
  そのときは「切り替えの前後で検索文字列とキャレット位置が保たれる」に読み替えます。**どちらに倒したかを
  実装時に記録**し、満たせない方の基準を黙って通さないこと。
...
- 2 種類を分けます。
  ...
  | | 効く範囲 | 個数 | 主な用途 |
  |---|---|---|---|
  | **グローバル** | どのアプリが前面でも | 絞る (OS の資源)。**既定は 3 つ** — summon / リーダー / シェルフのトグル。シェルフだけ直接キーなのは、掴んでいる手ではリーダーを打てないから ([`notch.md`](./notch.md) §4) | アプリ起動、クリップボード履歴、シェルフ |
...
- **リーダーキー**: グローバルは枯渇するので、`(リーダー) → 1 文字` の二段を用意します。既定 `Cmd+Opt+Space`。
  ...
  **割り当てを何個増やしてもグローバル登録が 3 つのまま**なのが要点で、これは Wayland で効きます — portal の
...
- リーダー + 1 文字で 30 個の割り当てを持っても、グローバル登録は **3 つのまま**。
...
id だけに張る設計では表現できません。OS へ実際に登録するのは summon・リーダー・シェルフのトグルの **3 つだけ**で、
...
| **P5** | `0.4.0` | プラグインストア (現行どおり) + **Raycast 拡張ソース互換 SDK** / OCR / AI (BYOK) / notch v2 (見張り + plugin の寄与 3 種。メディア・HUD・ウィジェットは**コアでは作らない** — [`notch.md`](./notch.md) §3.10) / **端末間共有 (シェルフ / クリップボード) (5.15、[`device-sharing.md`](./device-sharing.md))** / テーマ |
...
| **D8** | 同期 | (a) 提供しない (ファイル配置だけ同期可能にする) / (b) 自前クラウド / (c) **端末間の直接共有**。サーバにデータを置かない | **(c)**。シェルフとクリップボードは**同じ LAN の自分の端末どうし**で共有します ([`device-sharing.md`](./device-sharing.md))。(b) を採らないのは原則 1 で変わりません — 同期先はサーバではなく自分のもう一台で、**サーバは公開鍵の名簿を持つだけで中身を復号できません**。設定とプラグインは端末をまたがず ([§4.5](#45-コマンド拡張) の 5.12、`device-sharing.md` §6)、そこは (a) のままです |
...
| 項目 | 理由 |
|------|------|
| **独自クラウド同期** (サーバにユーザーデータを置く) | 原則 1。代わりに全データを iCloud Drive / Dropbox / git に置ける形にし、端末間の共有は**サーバを経由しない直接接続**で行います ([`device-sharing.md`](./device-sharing.md))。**アカウントそのものは作らないものではありません** — 無くても全機能が動く、というのが原則 1 の意味です |
| **チーム共有・SSO・MDM** | OSS 単体で背負うにはガバナンス要件が重すぎる。必要なら別レイヤ。**他人と共有する機能**の話で、自分の端末どうしの [`device-sharing.md`](./device-sharing.md) とは別です |
| **AI クレジットの販売** | 原則 1 |
...
**`R<n>` の番号は 4 文書 (本書・[`notch.md`](./notch.md)・[`migration.md`](./migration.md)・
[`device-sharing.md`](./device-sharing.md)) を通した 1 本の列です。** 本文からは番号だけで参照するので、
同じ番号を別の文書で使い回すと参照が壊れます。新しく足すときは 4 文書の最大値の次を取ってください。
複数の文書に関わる項目は、同じ番号で両方に載せます (R1 / R5 / R6 がそれです)。
現在の最大は **R29** ([`device-sharing.md`](./device-sharing.md) §14)。
