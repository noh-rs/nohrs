...
- **ピン留めも総量 2GB に含みます。** 「ピン留めは削除対象外」だけだと、全部ピン留めした時点で上限が消えます。
  削除できる未ピンが尽きたら、**新規クリップの保存を止め**、その旨を 1 行で伝えます (既存は消さない —
  ユーザーが明示的に残したものを、黙って捨てる方が悪い)。ピン留め自体にも上限 (既定 500MB) を置き、
  超える操作はピン留めを拒否します。
...
| 遷移 | レジストリの扱い |
|------|----------------|
| **load** | `list-commands` の結果を登録。`id` は `<plugin_id>.<command_id>` に名前空間化し、core との衝突を防ぐ |
...
(`cargo fmt --check` / `cargo clippy -D warnings` / `cargo test` が CI で green) が**そのまま適用**されます。
