...
- 取りに行くときの作法。**ページ本体だけでなく、`og:image` と favicon にも同じ規則を当てます** —
  ...
  - **名前解決して検証した IP に接続を固定します** (`Host` と SNI は元のホスト名のまま)。
    ...
    **接続先 IP を固定できない HTTP クライアントしか使えないなら、OGP 取得は作りません** (R9)。
...
| R7 | `wlr-foreign-toplevel-management` を出しているコンポジタの範囲 (wlroots 系のどこまで、GNOME / KDE の代替はあるか) | ウィンドウ単位のフォーカス履歴を Linux で出せるか ([§5.11](#511-ウィンドウとフォーカスの行き来))。無ければコマンドごと消す |
| R8 | gpui 0.2 でウィンドウを**上端を固定したままリサイズ**できるか (`List` ↔ `Split` の遷移) | できなければ 2 つのウィンドウを切り替える形になり、検索バーが動く ([§5.9](#59-ウィンドウのサイズと二枚組レイアウト)) |
| R9 | 名前解決した後の IP でリダイレクト先を都度検査できる HTTP クライアントが、依存を増やさずに書けるか | OGP 取得の SSRF 対策 (同 §5.9)。書けないなら D9 は (a) に倒す |
