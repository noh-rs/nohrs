...
| **notch のメディア / HUD / ウィジェット (コアとして)** | [`notch.md`](./notch.md) §3.0 の T4。Control Center が同じものを 20px 隣で出しており、狭い席を取る価値がない。加えて Now Playing は非公開 API 依存。置き場所は plugin (同 §3.9) とするが、これは**方針であって動作保証ではない**: 実際に作れるかは ① 公開 API で情報を取れるか (R7) ② host がその capability を plugin に渡すか、の両方が決まってから ([`notch.md`](./notch.md) §3.10) |
