...
- コマンドは次のメタデータを持つ: `id` / `title` / `subtitle` / `icon` / `keywords` / `category` / `mode` /
  ...
  **この一覧は [`launcher.md`](./launcher.md) §4.2 と [`plugin-api.md`](./plugin-api.md) の
  `command-info` の両方より広くなっています。** 3 つを揃えるまで、このレジストリ契約は確定していません —
  片方だけ実装すると core と plugin でコマンドの形が変わります。揃える作業は
  [§7.1](#71-command-レジストリの置き場所) の一覧のとおりで、**P3 のコマンドフレームワークに着手する前**に
  済ませます (本 PR は要件側だけを書いており、2 つの API 仕様はまだ直していません)。
