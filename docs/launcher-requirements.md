...
- クリップボード履歴は `layout = Split` です。左に一覧、右に選択中の中身
  ([§5.9](#59-ウィンドウのサイズと二枚組レイアウト))。750 × 500 の一覧に押し込むと、
  **1 件の中身を見るのに開き直すことになります**。
...
- ウィンドウのサイズは**ビューが宣言します**。ユーザーが毎回引き伸ばす作りにはしません。
  ...
  | `Split` | **1100 × 700** | 検索バー + **左 一覧 (380px) / 右 プレビュー (残り)** | クリップボード履歴、スニペット、最近のファイル、絵文字 |
