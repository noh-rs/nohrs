...
- 取りに行くときの作法:
  ...
  - **プライベート IP・ループバック・リンクローカル (`169.254.169.254` を含む) へは行きません。**
    ...
    クラウドのメタデータエンドポイントを踏ませる経路になり得ます。判定は**名前解決した後の IP** に対して
    行い、リダイレクト先も毎回判定します (DNS rebinding)。
  - 画像は縮小して `$XDG_CACHE_HOME/nohrs/ogp/` に置き、**30 日 / 合計 200MB** で切ります。
...
- **launcher を開いた瞬間のフォーカス元を覚え、コマンドが終わったらそこへ戻します。**
  貼り戻し ([§5.4](#54-クリップボード履歴)) が既にこれを要求しています。「戻り先」を 1 つの概念にしておくと、
  貼り戻し・ウィンドウ管理・スニペット展開が同じものを使えます。
