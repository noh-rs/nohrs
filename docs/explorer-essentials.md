...
### 1.6 symlink とコピー先の扱い

再帰コピーは symlink を **リンクとして複製する (辿らない)**。`cp -R` / Finder と同じ挙動で、リンク先がディレクトリでも祖先を指していてもコピーは完走し、ツリー外への読み書きも起こらない。

コピー/移動の宛先は services 側で 2 つの不変条件を守る (`nohrs-services::fs::ops`):

- 宛先がコピー元自身、またはその配下であってはならない。ディレクトリを自身の中へコピーすると無限再帰になり、ファイルを自身へコピーすると読む前に truncate される。
- 宛先名は単一の通常コンポーネントでなければならない (`destination_in`)。クリップボードやドロップから来た名前をそのまま `join` させない。

ペーストの書き込みはすべて**宛先を確保する**形 (`copy_path_no_replace` / `move_path_no_replace`) で行う。「存在確認 → 書き込み」の間に第三者が同名を作ると、置換する書き込みはそれを破壊するため。Overwrite は退避 → 確保付き書き込み → 失敗時ロールバックの順。

失敗時に宛先を片付けてよいかは、**エラーではなく操作自身が返す** (`ClaimFailure::left_a_partial_destination`)。`EEXIST` はパスを持たないため「宛先そのものが先に取られていた」と「自分が作ったツリーの中の名前が取られた」が区別できず、前者を後者と読むと他人のディレクトリを消す。消してよいのは**自分が確保して書きかけのまま落ちた宛先だけ**で、これはコピー元が無傷であることとセットで保証される。

除外される 2 つはどちらもデータを失う: 確保より前に落ちた失敗が残したものは他人のもの。そしてコピー完了後に**コピー元の削除が失敗した**場合、宛先にあるのは完全なコピーであり、`remove_dir_all` は原子的でないのでコピー元は既に一部欠けているかもしれない — 唯一の完全な複製を消すことになる。

片付けは `ClaimFailure::discard_partial_destination` を通す。確保した時点のエントリ識別 (`dev`/`ino`) を保持していて、書いている間に別プロセスが同名を取り直していたら何もしない。パス名だけで消すと相手のものを消すため。
...
選択は**行の添字ではなくパスの集合**で保持し、行番号は描画時に引く。ソート順やフィルタを変えても選択は選んだエントリに残り、「選んでから並べ替えて操作する」が成立する。見えなくなったエントリ (フィルタ除外、削除、別ディレクトリへ移動) は選択から落ちる。
