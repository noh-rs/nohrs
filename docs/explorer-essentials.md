...
スプリットは gpui-component の **dock** (`DockArea` / `TabPanel`) 上に構築する。
`ExplorerPane` が dock の `Panel` として各ペインを担い、`ExplorerPage` が
`DockArea` と共有サイドバーを所有する。

| 観点 | 仕様 | 備考 |
|------|------|------|
| 分割数 | **N-way** (dock が任意数のスプリット/タブを管理) | VSCode/Zed 相当 |
| 方向 | 水平・垂直 両方 (`Cmd+\\` で右に、`Cmd+Shift+\\` で下に分割) | — |
| タブ DnD | タブをドラッグしてスプリット内/間を移動、ペイン端へのドロップで分割 | dock 標準 |
| ドロップ分割 | フォルダ行・サイドバー項目をドックへドラッグ → 端プレビュー → 新タブ/スプリット | `ExplorerDrag` overlay |
| ペイン閉じる | タブの × ボタン。常に最低 1 pane は残る | — |
| タブバー | 1 スプリットに多数のタブがある場合は横スクロール | dock 標準 |

### 3.1.1 共有サイドバー

左の quick-access サイドバー (Home/フォルダショートカット) は **ペイン単位ではなく
エクスプローラ全体で 1 つ**。クリックは active pane を遷移させ、項目をドックへ
ドラッグすると新しいタブ/スプリットを開く。`Cmd/Ctrl+B` で開閉する。
...
**tab はスプリット単位** (各スプリット = dock の `TabPanel` が自身の tab bar を持つ)。
タブはドラッグでスプリット内/間を移動でき、ペイン端へドロップすると新しいスプリット
になる。サイドバーはスプリット間で共有 (§3.1.1)。
