...
失敗時に宛先を片付けてよいかは、**エラーではなく操作自身が返す** (`ClaimFailure::took_the_destination`)。`EEXIST` はパスを持たないため「宛先そのものが先に取られていた」と「自分が作ったツリーの中の名前が取られた」が区別できず、前者を後者と読むと他人のディレクトリを消す。確保より前に落ちた失敗は何も残さないので触らない。確保より後なら、そこにあるのは自分の書きかけであり、それを消すことがロールバックの前提になる。
