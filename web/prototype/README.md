...
TanStack Start への実装 ([`docs/web.md`](../../docs/web.md) §3) に着手する前に、デザインの合意を取るために置きました。

> **本番実装は [`web/`](../README.md) に入っています (2026-09-08)。**
> トークン・レイアウト・文言の正は本番側です。このプロトタイプは、**どの装飾を試して外したか**の記録として
> 残しています。同じ装飾を再発明しないために、下の「一度入れて、外したもの」を先に読んでください。
...
初版にはヒーロー背景の**全面**シェーダー、回転する `×`、カーソルに吸い付く CTA、
...
ただし後日、ヒーロー右の **fluid orb** としてシェーダーは戻しました。**面か形かが境界です** —
背景全面を覆う滲みは汚れに見えますが、円に閉じた orb は意図して置かれた物体として読めます。
ダークテーマでは地色寄りに振ると茶色い汚れに見えるため、ライトより強く振っています
(`--orb-strength` ライト `0.46` / ダーク `0.55`)。
